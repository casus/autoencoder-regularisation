...
wass_dist_matrix = np.zeros((testImages.shape[0], testImages.shape[0]))
for i in range(testImages.shape[0]):
    for j in range(testImages.shape[0]):
        if(i==j):
            wass_dist_matrix[j, i] = 0
        else:
            wassDistance = ot.sliced_wasserstein_distance(testImages[i][0], testImages[j][0], seed=0)  
...
imageIndices = indices_array(testImages.shape[3]).reshape(32*32,2)
...
for i in range(testImages.shape[0]):
    for j in range(testImages.shape[0]):
        if(i==j):
            ...
            gs = ((torch.tensor(testImages[i]) + 10**(-10)).reshape(1024,1)) / torch.sum((torch.tensor(testImages[i])))
            h = ((torch.tensor(testImages[j]) + 10**(-10)).reshape(1024,1)) / torch.sum((torch.tensor(testImages[j])))
...
dist_matrix_lat20 = wass_dist_matrix

dist_matrix_lat20 = torch.tensor(dist_matrix_lat20)
#dist_matrix_lat20 = _compute_distance_matrix(wass_dist_matrix, p=2)
