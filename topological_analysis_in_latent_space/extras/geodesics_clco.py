...
def get_potential_positions(dist_matrix, all_edges):
    ...
    for i in range(all_edges.shape[0]-1):
        ...
        edge_leads_to_loop = expecting_a_cycle( all_edges[:i+1], torch.tensor([[8., 45.]]))
...
for i in range(len(latent_dims)):
    rec_bAE_test = all_hyb_base_models[i].encoder(testImages.float())#.view(all_test_coeffs[i].shape)
...
dist_matrix_lat20 = _compute_distance_matrix(all_rec_bAE_test[3], p=2)
...
edge_collection = edges_lat20[:potent_posits+1]

print('edge_collection', edge_collection)

input_edges = torch.tensor([[0., 10.]])

loops = get_all_loops_formed(edge_collection, input_edges)[:-1]



print('loops', loops.flatten())

geodesic = initial =list(loops.flatten())

result = []
[result.append(item) for item in initial if item not in result]


print('geodesic', result)

print('len(result)', len(result))

Geodesic = [int(a) for a in result]
...
for k in Geodesic:
    ...
    plt.axis('off')
    #plt.savefig('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/topological_analysis_in_latent_space/geodesic_btw_Im0_Im10/MLP_AE/gamma'+str(t)+'value.png')
    plt.savefig('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/topological_analysis_in_latent_space/geodesic_btw_Im0_Im10/AE_REG/gamma'+str(t)+'value.png')
    plt.close()
    t = t+1
#print(result[0])