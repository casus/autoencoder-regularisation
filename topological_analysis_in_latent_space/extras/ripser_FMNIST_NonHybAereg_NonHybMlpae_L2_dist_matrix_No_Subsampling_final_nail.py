...
for i in range(len(latent_dims)):
    ...
    plt.xlabel('Birth',fontsize=15)
    plt.ylabel('Death' ,fontsize=15)
    plt.savefig('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/topological_analysis_in_latent_space/plots_from_topology/PH_FMNIST_HybAeRegLatDim'+str(latent_dims[i])+'_L2_dist_matrix_NoSubsampling_size200.png')
    plt.close()
...
for i in range(len(latent_dims)):
    dist_matrix_Lat = _compute_distance_matrix(all_rec_rAE_test[i], p=2)
    diagrams = ripser.ripser(dist_matrix_Lat.cpu().detach().numpy(), distance_matrix=True, maxdim=2)['dgms']
    signature_lat = diagrams[0][:-1]
    signature_ori = diagrams_ori[0][:-1]
    signature_lat_normal = (signature_lat - signature_lat.min()) / (signature_lat.max() - signature_lat.min()) 
    signature_ori_normal = (signature_ori - signature_ori.min()) / (signature_ori.max() - signature_ori.min()) 
    mse = np.mean(np.sqrt((signature_lat_normal - signature_ori_normal)**2))    

    #mse = np.mean(np.sqrt((diagrams[0][:-1] - diagrams_ori[0][:-1])**2))
    all_mse_aereg.append(mse)
    pre_diag = diagrams
figure(figsize=(8, 6), dpi=100)
plt.plot(latent_dims, all_mse_aereg)
plt.xticks(latent_dims, fontsize = 15)
plt.yticks(fontsize = 15)
plt.xlabel('Latent Dimension',fontsize=15)
plt.ylabel('L-2 Distance' ,fontsize=15)
plt.savefig('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/topological_analysis_in_latent_space/plots_from_topology/AllLatDimPH_distances_from_originalImageCloudPH_FMNIST_HybAeReg_L2_dist_matrix_NoSubsampling_size200.png')
plt.close()
...
for i in range(len(latent_dims)):
    ...
    plt.xlabel('Birth',fontsize=15)
    plt.ylabel('Death' ,fontsize=15)
    plt.savefig('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/topological_analysis_in_latent_space/plots_from_topology/PH_FMNIST_HybMlpaeLatDim'+str(latent_dims[i])+'_L2_dist_matrix_NoSubsampling_size200.png')
    plt.close()
...
for i in range(len(latent_dims)):
    ...
    mse = np.mean(np.sqrt((signature_lat_normal - signature_ori_normal)**2))
    #mse = np.mean(np.sqrt((diagrams[0][:-1] - diagrams_ori[0][:-1])**2))
    all_mse_mlp.append(mse)
    pre_diag = diagrams
figure(figsize=(8, 6), dpi=100)
plt.plot(latent_dims, all_mse_mlp)
plt.xticks(latent_dims, fontsize = 15)
plt.yticks(fontsize = 15)
plt.xlabel('Latent Dimension',fontsize=15)
plt.ylabel('L-2 Distance' ,fontsize=15)
plt.savefig('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/topological_analysis_in_latent_space/plots_from_topology/AllLatDimPH_distances_from_originalImageCloudPH_FMNIST_HybMlpae_L2_dist_matrix_NoSubsampling_size200.png')
plt.close()


figure(figsize=(8, 6), dpi=100)
plt.plot(latent_dims, all_mse_aereg, label = 'AE-REG')
plt.plot(latent_dims, all_mse_mlp, label = 'MLP-AE')

plt.legend(fontsize=14)

#plt.title('Direct Evaluation of Polynomials', fontsize=20)

plt.xticks(latent_dims, fontsize=15 ) 
plt.yticks(fontsize= 15) 
plt.xlabel('Latent Dimension',fontsize=16)
plt.ylabel('L-2 Distance',fontsize=16)
plt.savefig('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/topological_analysis_in_latent_space/plots_from_topology/NornalizedAllLatDimPH_distances_from_originalImageCloudPH_FMNIST_NonHybMlpaeHybAereg_L2_dist_matrix_NoSubsampling_size200.png')
plt.close()