...
print('image_batches_test.min()',image_batches_test.min())
...
class Autoencoder_linear(nn.Module):
    def __init__(self,latent_dim):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(input_dim, 6),  #input layer
            Sin(),
            nn.Linear(6, 6),    #h1
            Sin(),
            nn.Linear(6, 6),    #h1
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(6, 6),    #h1
            Sin(),
            nn.Linear(6, input_dim),  # latent layer
            nn.Sigmoid()
        )
...
torch.save(model.state_dict(), path+'/model_base_contraAE_1024tor24000'+name)