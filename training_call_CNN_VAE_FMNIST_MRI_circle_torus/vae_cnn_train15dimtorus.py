...
from vae import Flatten, UnFlatten, VAE, VAE3232input, ConvVAE_circle
...
z_dim = latent_dim
TDA = 1.0
model = ConvVAE_circle(image_channels=1, h_dim=5*4, z_dim=z_dim).to(device)
...
#image_batches_trn = image_batches_trn*0.5 + 0.5
...
torch.save(model.state_dict(), path+'/model_base_cnn_vae_tor200'+name)
