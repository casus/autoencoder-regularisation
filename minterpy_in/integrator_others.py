...
class gaussPoints:
    ...
    def __setPoints(self):
        if self.mode == "gaussLeg":
            ...
            polynomial = np.polynomial.laguerre.laggauss
        else:
    ...
    def transform(self, low, up):
        ...
        -----
            ...
            -   new gauss points and weights are
                ...
                where :math:`\hat x_i,\hat w_i` are the gauss points/weights to
                boundaries :math:`(a,b)` and :math:`x_i,w_i` the gauss points/weights to boundaries :math:`(-1,1)`
...
class ngauss_quad(object):
    def __init__(self, dim, num_pts):
        ...
        self.weights_nd = np.prod(np.array([t for t in it.product(*[self.weights_1d for _ in np.arange(dim)])]).T,
                                  axis=0)
