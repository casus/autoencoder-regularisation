...
class Autoencoder_DeepFluid(nn.Module):
    ...
    def get_intersize(self,inp_size):
        """
...
def get_intersize(inp_size,no_layers):
        """
        This function calculates the reduction of dimension through convolution. 
        The calculation is performed only on a single dimension
        inp_size: size of the input dimension. 
        """
        size = inp_size + 1 
        for i in range(no_layers - (inp_size%2)):
            size = int((size - 1)/2+1)
        return size - 1
    
def get_flintersize(inp_size, no_layers, no_channels):
        """
        This function calculates the size of the latent space after the convolutional layer. 
        It uses the get_intersize function in order to calculate the dimensional reduction through convolution. 
        """
        size_x = get_intersize(inp_size[0], no_layers)
        size_y = get_intersize(inp_size[1], no_layers)
        latent_in_features = size_x * size_y * no_channels
        return latent_in_features
...
class ConvEncoder(torch.nn.Module):
    def __init__(self, inp_size, noChannels, channel_mult, no_layers,
        ...
        self.linlayer = nn.Linear(get_flintersize(inp_size, self.no_layers, channel_mult*(2**no_layers)), latent_dim)
...
class ConvDecoder(torch.nn.Module):
    def __init__(self, out_size, noChannels, channel_mult, no_layers,
        ...
        self.linlayer = nn.Linear(latent_dim, get_flintersize(out_size, no_layers, channel_mult*(2**no_layers)))
        
        self.convlayers = nn.ModuleList([])
        self.convlayers.append(nn.ConvTranspose2d(channel_mult*(2**no_layers), channel_mult*(2**no_layers),
                                 kernel_size=(kernel_size, kernel_size), stride=1, padding=1, bias=False))
        
        self.normlayers = nn.ModuleList([])
        self.normlayers.append(nn.BatchNorm1d(get_flintersize(out_size, no_layers, channel_mult*(2**no_layers))))
        
        for i in range(self.no_layers, 0, -1):
        ...
        self.normlayers.append(nn.BatchNorm2d(channel_mult))
        self.activation = activation
          
    def forward(self, x):
        x = self.activation(self.normlayers[0](self.linlayer(x)))
        x = x.reshape(x.size()[0], self.channel_mult*(2**self.no_layers), 
                      get_intersize(self.out_size[0], self.no_layers), 
                      get_intersize(self.out_size[1], self.no_layers))
        for i in range(self.no_layers+1):
            x = self.activation(
                self.normlayers[i+1](
                    self.convlayers[i](x)
                ))
            x = torch.nn.functional.interpolate(x, size=(get_intersize(self.out_size[0], self.no_layers-i),
                                                         get_intersize(self.out_size[1], self.no_layers-i)), mode='bilinear')
        x = torch.sigmoid(
            self.convlayers[-1](x)
            )
        return x        
...
class ConvAE(torch.nn.Module):
    def __init__(self, inp_size, noChannels, channel_mult, no_layers, kernel_size, latent_dim, activation):
        super(ConvAE, self).__init__()
        
        self.encoder = ConvEncoder(inp_size, noChannels, channel_mult, no_layers, kernel_size, latent_dim, activation)
        self.decoder = ConvDecoder(inp_size, noChannels, channel_mult, no_layers, kernel_size, latent_dim, activation)
