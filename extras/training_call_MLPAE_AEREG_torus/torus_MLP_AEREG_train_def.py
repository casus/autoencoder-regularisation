...
def train(train_loader, test_loader, no_channels, dx, dy, no_epochs=2, TDA=0.4, reco_loss='mse', latent_dim=10, 
    ...
    image_batches_trn = torch.load('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/circle_dataset/15_dim_torus_2000_nonNormalized.pt').to(device)
    ...
    image_batches_test = torch.load('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/circle_dataset/15_dim_torus_2000_nonNormalized.pt').to(device)
    ...
    if train_base_model:
        ...
        torch.save(loss_arr_reg, path+'/loss_arr_reg_tor200'+name)
        torch.save(loss_arr_reco, path+'/loss_arr_reco_tor200'+name)
        torch.save(loss_arr_base, path+'/loss_arr_base_tor200'+name)
        torch.save(loss_arr_val_reco, path+'/loss_arr_val_reco_tor200'+name)
        torch.save(loss_arr_val_base, path+'/loss_arr_val_base_tor200'+name)
        torch.save(model.state_dict(), path+'/model_base_tor200'+name)
        torch.save(model_reg.state_dict(), path+'/model_reg_tor200'+name)
