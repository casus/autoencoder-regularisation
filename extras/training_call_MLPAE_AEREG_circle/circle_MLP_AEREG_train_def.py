...
def train(train_loader, test_loader, no_channels, dx, dy, no_epochs=2, TDA=0.4, reco_loss='mse', latent_dim=10, 
    ...
    if train_base_model:
        #model = AE_circ(latent_dim).to(device)
        model = copy.deepcopy(model_reg)'''
    ...
    if train_base_model:
        ...
        torch.save(loss_arr_reg, path+'/loss_arr_reg_circ'+name)
        torch.save(loss_arr_reco, path+'/loss_arr_reco_circ'+name)
        torch.save(loss_arr_base, path+'/loss_arr_base_circ'+name)
        torch.save(loss_arr_val_reco, path+'/loss_arr_val_reco_circ'+name)
        torch.save(loss_arr_val_base, path+'/loss_arr_val_base_circ'+name)
        torch.save(model.state_dict(), path+'/model_base_circ'+name)
        torch.save(model_reg.state_dict(), path+'/model_reg_circ'+name)
