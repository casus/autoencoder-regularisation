...
from models_of_VAEs_circle import BetaVAE, MLPVAE, ConvVAE_circle, VAE_mlp_circle_new, ConvVAE_circle1024
...
model_reg.load_state_dict(torch.load(path_unhyb+'model_reg_1024tor24000'+name_unhyb, map_location=torch.device(device)))
model_base.load_state_dict(torch.load(path_unhyb+'model_base_1024tor24000'+name_unhyb, map_location=torch.device(device)))


'''print('check Loaded? ')
# 3 training points
training_points = torch.load('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/torus_dataset/1024_dim_torus_1000pts.pt').to(device)
training_points = training_points.cpu()
# Generate 15 dim circle and normalize it
num_pts = training_points.shape[0]
radius = 1.0
...
#fifteen_dim_sphere_train = torch.load('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/torus_dataset/1024_dim_torus_1000pts.pt')

########################################################

fifteen_dim_sphere_train = torch.load('/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/circle_dataset/3_dim_torus_2000_nonNormalized.pt')

num_pts = fifteen_dim_sphere_train.shape[0]
A_transform5 = np.random.uniform(-1.0, 1.0, 3*1024).reshape(3, 1024)
A_trans_torus = torch.tensor(A_transform5)



fifteen_dim_sphere_train = torch.matmul(fifteen_dim_sphere_train, A_trans_torus.double())
...
torch.save(rec_rAE, '/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/1024dim_torus_embeddings/embedded_point_cloud_saved/24000trainingPoints1000testPoints/rec_rAE.pt')

torch.save(rec_bAE, '/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/1024dim_torus_embeddings/embedded_point_cloud_saved/24000trainingPoints1000testPoints/rec_bAE.pt')
...
from circle_all_models import ConvoAE_torus1024
no_layers_cae = 2
latent_dim_cae = latent_dim
lr_cae =0.002
name_unhyb_cae = '_'+str(frac)+'_'+str(latent_dim_cae)+'_'+str(lr_cae)+'_'+str(no_layers_cae)
model_convAE = ConvoAE_torus1024(latent_dim_cae).to(device)
model_convAE.load_state_dict(torch.load(path_unhyb+'model_base_cae_1024tor24000'+name_unhyb_cae, map_location=torch.device(device)), strict=False)


print("loadin cricle cnn ?")


model_convAE = model_convAE.encoder(batch_x.reshape(num_pts,1,inp_dim_unhyb).float()).detach().cpu().numpy() 

#rec_bAE = model_base.encoder(batch_x.float()).detach().cpu().numpy() 

print('model_convAE.shape', model_convAE.shape)
torch.save(model_convAE, '/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/1024dim_torus_embeddings/embedded_point_cloud_saved/24000trainingPoints1000testPoints/model_convAE.pt')
...
model_contra.load_state_dict(torch.load(path_unhyb+'model_base_contraAE_1024tor24000'+name_unhyb_contraae, map_location=torch.device(device)), strict=False)

print("loading ?")

model_contra = model_contra.encoder(batch_x.reshape(num_pts,1,inp_dim_unhyb).float()).detach().cpu().numpy() 
#rec_bAE = model_base.encoder(batch_x.float()).detach().cpu().numpy() 
model_contra = model_contra.reshape(num_pts,3)
print("contra shape ", model_contra.shape)

torch.save(model_contra, '/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/1024dim_torus_embeddings/embedded_point_cloud_saved/24000trainingPoints1000testPoints/model_contra.pt')
...
model_mlpVAE = MLPVAE(1024, hidden_size, latent_dim, 
...
model_mlpVAE.load_state_dict(torch.load(path_unhyb+'model_base_mlp_vae_1024tor24000'+name_unhyb_vae, map_location=torch.device(device)), strict=False)

model_betaVAE = ConvVAE_circle1024(image_channels=1, h_dim=256, z_dim=latent_dim).to(device)
model_betaVAE.load_state_dict(torch.load(path_unhyb+'model_base_cnn_vae_1024tor24000'+name_unhyb_vae, map_location=torch.device(device)), strict=False)
...
model_mlpVAE = model_mlpVAE.fc1(model_mlpVAE.encoder(batch_x.reshape(num_pts,1,image_size).float())).reshape(num_pts,3).detach().cpu().numpy() 

print('model_mlpVAE.shape', model_mlpVAE.shape)

torch.save(model_mlpVAE, '/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/1024dim_torus_embeddings/embedded_point_cloud_saved/24000trainingPoints1000testPoints/model_mlpVAE.pt')
...
model_betaVAE = model_betaVAE.fc1(model_betaVAE.encoder(batch_x.reshape(num_pts,1,image_size).float())).detach().cpu().numpy() 
...
torch.save(model_betaVAE, '/home/ramana44/topological-analysis-of-curved-spaces-and-hybridization-of-autoencoders/1024dim_torus_embeddings/embedded_point_cloud_saved/24000trainingPoints1000testPoints/model_betaVAE.pt')
